...
const os = require("os");
function getDefaultDocumentsFolder() {
    const home = os.homedir();
    return path.join(home, 'Documents');
}
async function getDefaultTemplate(context, templateFileName) {
...
function activate(context) {
    ...
    const disposable = vscode.commands.registerCommand('quick-notes.openNote', async () => {
        try {
            ...
            let notesDir = config.get('notesDirectory');
            const templateFileName = config.get('templateFileName') || 'quick-notes-template.md';
            let defaultTemplate = await getDefaultTemplate(context, templateFileName);
            const fileNameFormat = config.get("fileNameFormat") || 'Quick-Note-{{DATE}}';
            const fileExtension = config.get('fileExtension') || '.txt';
            ...
            for (const char of dateFormat) {
                switch (char) {
                    case 'D':
                        const curDayPart = day[dayIndex] ? day[dayIndex] : '';
                        dateString += curDayPart;
                        dayIndex++;
                        break;
                    case 'M':
                        const curMonthPart = month[monthIndex] ? month[monthIndex] : '';
                        dateString += curMonthPart;
                        monthIndex++;
                        break;
                    case 'Y':
                        const curYearPart = year[yearIndex] ? year[yearIndex] : '';
    ...
    const statusBarItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, // or Right
